...
文档规则：`doc/` 下生成的所有文档必须使用中文撰写。每次完成产品修复后，都必须在 `doc/fix/` 下生成对应的中文 Markdown 修复报告，并遵循现有修复报告的结构。新增产品功能应使用独立的文档分类，例如 `doc/feat/`；目录不存在时应创建。实现说明、验证结果、已知限制和用户验证步骤必须写入对应文档，不得提交临时草稿或生成的无关产物。
