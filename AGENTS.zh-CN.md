...
- 非 Rust 文本文件出现 CRLF、行尾损坏，或 diff 仅由行尾变化造成时，必须停止自动编辑并人工处理。保留文件内容，仅将行尾规范化为 LF，并通过字节级检查和 `git diff --check` 验证工作区及暂存区。
- 除非人工修复不可行，否则不得使用脚本批量重写行尾。仅执行 `git add filename` 不视为修复完成。

文档规则：`doc/` 下生成的所有文档必须使用中文撰写。每次完成产品修复后，都必须在 `doc/fix/` 下生成对应的中文 Markdown 修复报告，并遵循现有修复报告的结构。新增产品功能应使用独立的文档分类，例如 `doc/feat/`；目录不存在时应创建。实现说明、验证结果、已知限制和用户验证步骤必须写入对应文档。`doc/` 下的文档默认仅作为本地记录，不得加入暂存区或提交，除非项目负责人明确要求提交。这包括 `doc/fix/` 修复报告和 `doc/question/` 工具失败报告。
...
仓库根目录已配置 `rustfmt.toml`，其中包含项目格式化策略以及 Unix 行尾设置。当 Rust 文件出现行尾序列或格式差异时，直接执行 `cargo fmt --all`，不要手工改写行尾，也不要绕过项目配置的格式化工具。
