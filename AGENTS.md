...
Documentation policy: every document generated under `doc/` must be written in Chinese. Every completed product fix must have a corresponding Chinese Markdown repair report under `doc/fix/`, following the existing repair-report structure. New product features should use a dedicated documentation category such as `doc/feat/`; create the category when it does not exist. Keep implementation notes, validation results, known limitations, and user verification steps in the appropriate document. Documents under `doc/` are local records by default and must not be staged or committed unless the project owner explicitly requests their submission. This includes `doc/fix/` repair reports and `doc/question/` tool-failure reports.
